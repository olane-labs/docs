...
Your raw text never leaves your control unencrypted. Olane uses client-side AES-256-GCM encryption so that the data you send is encrypted before it reaches our servers, processed transiently in memory for extraction, and archived in its original encrypted form. The server never stores your plaintext.
...
When you send data to Olane for extraction:

1. **You encrypt it** on your machine using a key only you possess
2. **We decrypt it** in server memory for the duration of extraction
...
The extracted knowledge graph (entities, behaviors, containment relationships) is stored in the database. The raw text that produced it is not.
...
```bash
python3 .olane/scripts/olane_crypto.py generate-key
```

This produces a UUID4 string and saves it to `.olane/config.json`. This key never leaves your machine. If you lose it, archived encrypted data cannot be recovered — there is no key escrow.
...
# Encrypt text
encrypted = crypto.encrypt("Your sensitive text here")
# → {"encrypted_text": "b64...", "encryption_iv": "b64...", "encryption_tag": "b64..."}

# Encrypt a file
result = crypto.encrypt_file("/path/to/document.txt")
...
```bash
# Encrypt a JSON payload from stdin
echo '{"text":"Your sensitive text"}' | python3 .olane/scripts/olane_crypto.py encrypt --stdin

# Encrypt a file
python3 .olane/scripts/olane_crypto.py encrypt-file --path /path/to/file
```
...
Here is what happens when you send encrypted data to the `/extract` endpoint:

### Step 1: Client-Side Encryption

Your client encrypts the plaintext and prepares the request:

```python
crypto = OlaneCrypto(copass_encryption_key=os.environ["COPASS_ENCRYPTION_KEY"])
encrypted = crypto.encrypt("Meeting notes from standup...")

# Create session token for secure DEK transport
session_token = crypto.create_session_token(access_token)
```
...
The plaintext is processed through the extraction pipeline — LLM-based frame-semantic extraction produces structured entities, behaviors, and containment relationships. These are written to the database.
...
- **Extracted graph structure**: Entities, behaviors, and containment relationships derived from your text are stored unencrypted in the database. These are structured abstractions, not the original text.
- **Transient server memory**: During extraction, the plaintext exists in server memory. This is the trust boundary — you trust the server to process your data honestly for the duration of the request.
- **Metadata**: Source type, timestamps, and extraction IDs are stored in plaintext.
...
1. Generate a new key with `python3 .olane/scripts/olane_crypto.py generate-key` (into a new config)
2. All new extractions will use the new DEK derived from the new master key
...
All cryptographic operations use the Python `cryptography` library's hazmat primitives.
...
# Encrypt
encrypted = crypto.encrypt("your text")

# Get session token for API calls
session_token = crypto.create_session_token(access_token)

# Get raw DEK (for X-Encryption-Key header, legacy)
dek_b64 = crypto.dek_b64
```
...
# Encrypt from stdin
echo '{"text":"hello"}' | python3 .olane/scripts/olane_crypto.py encrypt --stdin

# Get DEK for header
DEK=$(python3 .olane/scripts/olane_crypto.py get-dek)

# Generate session token
TOKEN=$(python3 .olane/scripts/olane_crypto.py generate-session-token)
```
...
Archived encrypted data becomes permanently unrecoverable. The extracted graph (entities, behaviors, containment) remains accessible since it is stored separately. Generate and back up your master key securely.
...
**Is the extracted graph encrypted?**
No. The knowledge graph (entities, behaviors, containment relationships) is stored unencrypted. These are structured abstractions — they capture *what* was observed, not the original text. The raw text that produced them is what's protected.

**What if the server is compromised during extraction?**
