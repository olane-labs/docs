...
The structured output (entities, behaviors, containment relationships) is stored in the database. The raw text that produced it is not.
...
The plaintext is processed through the analysis engine to produce structured entities, behaviors, and containment relationships. These are written to the database.
...
Archived encrypted data becomes permanently unrecoverable. The structured output (entities, behaviors, containment) remains accessible since it is stored separately. Generate and back up your master key securely.
...
No. The structured output (entities, behaviors, containment relationships) is stored unencrypted. These are structured abstractions — they capture *what* was observed, not the original text. The raw text that produced them is what's protected.
