...
Teach Claude **why**, not just **what**.

Claude Code plugin for [Olane](https://olane.com).

<div style={{ maxWidth: '100%', margin: '1.5rem 0', padding: '1.25rem', border: '1px solid #E5E7EB', borderRadius: '12px' }}>
...
```bash
olane copass question <question>   # ask a question about the ontology
olane copass context               # get cosync context for the current task
olane copass score                 # get cosync scores for entities
```

### Watch

```bash
olane watch                 # start continuous indexing — watches for file changes and re-indexes automatically
```

### API Key Management

```bash
olane api-key               # manage API keys for programmatic access
```

### Ingest Flags

The `olane ingest` commands accept additional flags:

```bash
olane ingest text /path/to/file.txt \
  --stdin               # read from stdin instead of a file \
  --language <lang>     # specify language (e.g. "en") \
  --additional-context <ctx>  # extra context to associate with the ingestion \
  --project-id <id>     # target a specific project \
  --json                # output results as JSON \
  --source-type <type>  # classify the source (e.g. "meeting_notes", "documentation") \
  --entity-hints <hints> # hint at expected entities to improve extraction
```
